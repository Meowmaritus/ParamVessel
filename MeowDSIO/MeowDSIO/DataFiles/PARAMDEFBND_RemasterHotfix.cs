...
{
    public class PARAMDEFBND_RemasterHotfix : DataFile, IList<PARAMDEFBNDEntry>
    {
        const string PARAMDEF_DIR = @"N:\FRPG\data\INTERROOT_x64\paramdef";
        ...
        protected override void Read(DSBinaryReader bin, IProgress<(int, int)> prog)
        {
            ...
            foreach (var bndEntry in bnd)
            {
                try
                {
                    ...
                    Add(new PARAMDEFBNDEntry(paramDef.ID, paramDef));
                }
                catch
                {
