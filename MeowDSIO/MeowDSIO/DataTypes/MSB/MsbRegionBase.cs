...
namespace MeowDSIO.DataTypes.MSB
{
    public abstract class MsbRegionBase : MsbStruct
    {
        ...
        {
            bin.Placeholder32($"POINT_PARAM_ST|{Type}|{nameof(Name)}");
            ...
            bin.Placeholder32($"POINT_PARAM_ST|{Type}|(SUBTYPE DATA OFFSET 1)");
            bin.Placeholder32($"POINT_PARAM_ST|{Type}|(SUBTYPE DATA OFFSET 2)");
            bin.Placeholder32($"POINT_PARAM_ST|{Type}|(SUBTYPE DATA OFFSET 3)");
            bin.Placeholder32($"POINT_PARAM_ST|{Type}|(SUBTYPE DATA OFFSET 4)");
            ...
            //bin.StartMSBStrings();
            {
                bin.Replace32($"POINT_PARAM_ST|{Type}|{nameof(Name)}", bin.MsbOffset);
            ...
            bin.Replace32($"POINT_PARAM_ST|{Type}|(SUBTYPE DATA OFFSET 1)",
                msbOffset);

            bin.Replace32($"POINT_PARAM_ST|{Type}|(SUBTYPE DATA OFFSET 2)", 
                OffsetDeltas.Item1 >= 0 ? msbOffset + OffsetDeltas.Item1 : 0);

            bin.Replace32($"POINT_PARAM_ST|{Type}|(SUBTYPE DATA OFFSET 3)", 
                OffsetDeltas.Item2 >= 0 ? msbOffset + OffsetDeltas.Item2 : 0);

            bin.Replace32($"POINT_PARAM_ST|{Type}|(SUBTYPE DATA OFFSET 4)", 
