...
namespace MeowDSIO.DataTypes.MSB.MODEL_PARAM_ST
{
    public abstract class MsbModelBase : MsbStruct
    {
        ...
        {
            bin.Placeholder32($"MODEL_PARAM_ST|0|{nameof(Name)}");
            bin.Write((int)ModelType);
            bin.Write(Index);
            bin.Placeholder32($"MODEL_PARAM_ST|0|{nameof(PlaceholderModel)}");
            bin.Write(InstanceCount);
            bin.Write(BASE_CONST_1);
            bin.Write(BASE_CONST_2);
            bin.Write(BASE_CONST_3);

            bin.Replace32($"MODEL_PARAM_ST|0|{nameof(Name)}", bin.MsbOffset);
            bin.WriteMsbString(Name, terminate: true);

            bin.Replace32($"MODEL_PARAM_ST|0|{nameof(PlaceholderModel)}", bin.MsbOffset);
