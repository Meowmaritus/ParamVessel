...
namespace MeowDSIO.DataTypes.MSB
{
    public abstract class MsbEventBase : MsbStruct
    {
        ...
        {
            bin.Placeholder32($"EVENT_PARAM_ST|{Type}|Name");
            ...
            bin.Placeholder32($"EVENT_PARAM_ST|{Type}|(BASE DATA OFFSET)");
            bin.Placeholder32($"EVENT_PARAM_ST|{Type}|(SUBTYPE DATA OFFSET)");

            bin.Write(BASE_CONST_1);

            //bin.StartMSBStrings();
            {
                bin.Replace32($"EVENT_PARAM_ST|{Type}|Name", bin.MsbOffset);
            ...
            bin.Replace32($"EVENT_PARAM_ST|{Type}|(BASE DATA OFFSET)", bin.MsbOffset);
            bin.Write(i_Part);
            bin.Write(i_Region);
            bin.Write(EntityID);
            ...
            bin.Replace32($"EVENT_PARAM_ST|{Type}|(SUBTYPE DATA OFFSET)", bin.MsbOffset);
            SubtypeWrite(bin);
        }
