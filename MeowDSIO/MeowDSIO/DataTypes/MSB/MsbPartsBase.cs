...
namespace MeowDSIO.DataTypes.MSB
{
    public abstract class MsbPartsBase : MsbStruct
    {
        ...
        {
            bin.Placeholder32($"PARTS_PARAM_ST|{Type}|{Index}|{nameof(Name)}");
            ...
            bin.Placeholder32($"PARTS_PARAM_ST|{Type}|{Index}|{nameof(PlaceholderModel)}");
            ...
            bin.Placeholder32($"PARTS_PARAM_ST|{Type}|{Index}|(BASE DATA OFFSET)");
            bin.Placeholder32($"PARTS_PARAM_ST|{Type}|{Index}|(SUBTYPE DATA OFFSET)");
            ...
            bin.StartMSBStrings();
            {
                bin.Replace32($"PARTS_PARAM_ST|{Type}|{Index}|{nameof(Name)}", bin.MsbOffset);
                bin.WriteMsbString(Name, terminate: true);

                bin.Replace32($"PARTS_PARAM_ST|{Type}|{Index}|{nameof(PlaceholderModel)}", bin.MsbOffset);
                bin.WriteMsbString(PlaceholderModel, terminate: true);
            }
            bin.EndMSBStrings(blockSize);

            bin.Replace32($"PARTS_PARAM_ST|{Type}|{Index}|(BASE DATA OFFSET)", bin.MsbOffset);
            ...
            bin.Replace32($"PARTS_PARAM_ST|{Type}|{Index}|(SUBTYPE DATA OFFSET)", bin.MsbOffset);
            SubtypeWrite(bin);
        }
